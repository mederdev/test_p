...
export class OrdersController {
	...
	async newCarOrder(@Body() dto: OrderDto) {
		...
		await console.log(carAvailables(db));
		if (carInfo[1] == 't') {
			const resMessage = getPrice(dto.startDate, dto.endDate);
			if (typeof resMessage === 'string') return resMessage;
			const writedData = await db.query(`INSERT INTO ORDERS 
			(carID,carName,startDate,endDate,price,days) 
			VALUES('${dto.id}','${carInfo[0]}','${dto.startDate}',
			'${dto.endDate}','${resMessage}','1'+DATE_PART('day','${dto.endDate}'::timestamp-'${dto.startDate}'::timestamp))`);
